...
class MpwikFormatter(GenericDataFormatter):
    ...
    def get_fixed_params(self):
        ...
        fixed_params = {
            ...
            'num_encoder_steps': 6*25+7,    # Length of LSTM decoder (ie. # historical inputs)
...
model_folder = os.path.join(output_folder, 'saved_models', 'mpwik', 'ai-277')
...
class TFTEvaluator():
  ...
  def plot_predictions(self, t='t+0', start=0, end=None):
    plt.figure(figsize=(15,5))
    if end is None:
      end=self.output_map_formatted['targets'][t].shape[0]
    plt.plot(self.output_map_formatted['targets'][t].iloc[start:end], label='targets')
    plt.fill_between(self.output_map_formatted['p10'][t].iloc[start:end].index, self.output_map_formatted['p10'][t].iloc[start:end], self.output_map_formatted['p90'][t].iloc[start:end], label='p10-p90', color='g', alpha=.3)
    plt.plot(self.output_map_formatted['p50'][t].iloc[start:end], label='p50')
    plt.legend()
    plt.show();
```
