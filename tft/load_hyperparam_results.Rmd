...
class TftDistrHyperParamInterpreter(TftHyperParamInterpreter):
  ...
  def get_list_of_distr_results_folders(self):
    hyperparam_folder_names = [f for f in os.listdir(self.results_folder) if f not in ['tmp', 'hyperparams']]
...
interp = TftDistrHyperParamInterpreter('/home/pdybczak/tft_outputs_distr/saved_models/mpwik/pd_hyperparam_opt1/')
# interp = TftDistrHyperParamInterpreter('/home/pdybczak/tft_outputs_distr/saved_models/mpwik/main/')
```

```{python}
interp.get_list_of_distr_results_folders()
```

```{python}
interp.results.shape
```

```{python}
interp.board_info.index = [f"run_{i}" for i in range(interp.board_info.shape[0])]
