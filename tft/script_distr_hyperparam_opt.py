...
DistributedHyperparamOptManager = libs.hyperparam_opt.DistributedHyperparamOptManager
ModelClass = libs.tft_model.TemporalFusionTransformer


def main(expt_name, use_gpu, restart_opt, model_folder, hyperparam_iterations,
         data_csv_path, data_formatter, worker_number):
  ...
  opt_manager = DistributedHyperparamOptManager(param_ranges, fixed_params, model_folder, worker_number)
...
if __name__ == "__main__":
  ...
  def get_args():
    ...
    parser.add_argument(
        "worker_number",
        default=0,
        help="from 0 to length of cuda_visible_devices-1")
    ...
    return args.expt_name, root_folder, args.use_gpu == "yes", \
        args.restart_hyperparam_opt, args.worker_number

  # Load settings for default experiments
  name, folder, use_tensorflow_with_gpu, restart, worker_number = get_args()
